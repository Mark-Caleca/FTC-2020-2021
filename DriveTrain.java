...
public class DriveTrain {
    ...
    public void setPower(double speedAll){
        rightFrontMotor.setPower(speedAll);
        leftFrontMotor.setPower(speedAll);
        rightBackMotor.setPower(speedAll);
        leftBackMotor.setPower(speedAll);
    }
    ...
    //so apparently this is legal: https://en.wikipedia.org/wiki/Function_overloading
    //can be different type or different number of parameters https://www.programiz.com/java-programming/method-overloading#:~:text=In%20Java%2C%20two%20or%20more,func()%20%7B%20...%20%7D
...
//            error = 0;
    ...
    //public boolean allMotorsBusy ((rightFrontMotor.isBusy() && leftFrontMotor.isBusy() && rightBackMotor.isBusy() && leftBackMotor.isBusy())) ? true : false;
    public boolean allMotorsBusy(){
        boolean defaultState = false;
        if (rightFrontMotor.isBusy() && leftFrontMotor.isBusy() && rightBackMotor.isBusy() && leftBackMotor.isBusy()) {
            defaultState = true;
        }
        else { defaultState = false; }
        return defaultState;
    }
